...
interface ldl_solve
  module procedure ldl_solve_r8,ldl_solve_r4
end interface


contains

subroutine ldl_solve_r8(a,b,x,h)
...
real (kind=real_kind) :: t_norm,last_tnorm,best_tnorm
...
subroutine ldl_solve_r4(a,b,x,h)

type (hyper_dual), intent(inout) :: a(:,:)
type (hyper_dual), intent(in) :: b(:,:)
real (kind=real_32bit_kind), intent(out) :: x(:,:,0:)
type (hyper_dual), intent(in) :: h
...
type (hyper_dual) :: part_a(size(a,1),size(a,2))
type (hyper_dual) :: resid(size(b,1),size(b,2))
real (kind=real_32bit_kind) :: norm(1:4,size(b,2)) 
real (kind=real_32bit_kind) :: t_norm,last_tnorm,best_tnorm
real (kind=real_32bit_kind) :: r_r,r_e1,r_e1e2,r_e1e2e3
real (kind=real_32bit_kind) :: last_x(size(x,1),size(x,2),0:3),diff
...
do k=1,n
  ...
  do j=1,m
    t_norm=t_norm+sum(norm(:,j))
  enddo
  if ((t_norm.gt.2e0*last_tnorm).and.(k.gt.2)) then
    do j=1,m
      do i=1,n
        x_vp=best_y(i,j)%r
        x(i,j,0)=x_vp
