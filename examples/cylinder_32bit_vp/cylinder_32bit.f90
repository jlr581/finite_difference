...
real (kind=real_32bit_kind) :: pi,rad,dist,nu
real (kind=real_32bit_kind), allocatable :: min_dist(:)
real (kind=real_32bit_kind), allocatable :: u(:),v(:),p(:)
real (kind=real_32bit_kind), allocatable :: neigb(:,:),local_var(:)
real (kind=real_32bit_kind), allocatable :: a(:,:),b(:),d(:,:),e(:),x(:),work(:)
real (kind=real_32bit_kind) :: dr,dt,theta,txx,txy,theta1
real (kind=real_32bit_kind), allocatable :: wsum(:),csum(:,:)
integer :: i1,i2

character(7) :: derivative(5)
integer, allocatable :: coeff_neigh(:,:)
integer, allocatable :: coeff_num_neigh(:)
real (kind=real_32bit_kind), allocatable :: points(:,:)
integer, allocatable :: coeff_prec(:)
real (kind=real_32bit_kind), allocatable :: coeff(:,:,:)
...
call sgglse(3*n,3*n,2*(nc+nb)+1,a,3*n,d,2*(nc+nb)+1,b,e,x,work,lwork,info)
if (info/=0) then
  print *,'error in soln'
  stop
endif
...
do i=1,nc
  ...
  enddo
  csum(i,1)=kfold_dot_product(real(coeff(i,1:np,1),kind=real_kind), &
          real(local_var(1:np),kind=real_kind))
  csum(i,2)=kfold_dot_product(real(coeff(i,1:np,2),kind=real_kind), &
          real(local_var(1:np),kind=real_kind))
  do j=1,np
    local_var(j)=v(coeff_neigh(i,j))
  enddo
  csum(i,3)=kfold_dot_product(real(coeff(i,1:np,1),kind=real_kind), &
          real(local_var(1:np),kind=real_kind))
